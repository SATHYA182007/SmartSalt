...
import { Zap, Droplets, Thermometer, ArrowRight } from 'lucide-react';
...
const createCustomMarkerIcon = (block: SaltBlock, isSelected: boolean) => {
  ...
  const ringStyle = isSelected ? 'box-shadow: 0 0 0 4px #2563EB, 0 8px 16px rgba(37,99,235,0.4); transform: scale(1.15);' : 'box-shadow: 0 4px 10px rgba(0,0,0,0.25);';
...
export const LeafletSaltMap: React.FC<LeafletSaltMapProps> = ({
  ...
  // Center of salt field area (Salinas coordinates near coastal evaporation pans)
  const centerLat = selectedBlock ? selectedBlock.lat : 36.654;
  const centerLng = selectedBlock ? selectedBlock.lng : -6.295;

  return (
    <div className="w-full h-[400px] sm:h-[480px] rounded-2xl overflow-hidden border border-slate-200 shadow-xs relative">
      <MapContainer
      ...
      >
        ...
        <TileLayer
          attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors &copy; <a href="https://www.esri.com/">Esri Satellite</a>'
